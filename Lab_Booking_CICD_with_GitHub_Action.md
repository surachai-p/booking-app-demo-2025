...
```plaintext
# ตอบที่นี่
CI คือการตรวจโค้ดอัตโนมัติ ทุกครั้งที่ push งานขึ้น github เพื่อดูว่าโค้ดใหม่ทำระบบพังไหม

CD คือการเอาโค้ดที่ตรวจผ่านแล้วไปลงหน้าเว็บจริง ให้คนใช้งานทันทีแบบอัตโนมัติ

ในโปรเจกต์นี้ เมื่อ push ไปที่ branch main ตัว github actions จะช่วยเช็คความเรียบร้อย และส่งไปที่ render/vercel
...
```plaintext
# ตอบที่นี่
ข้อดี: แยกกันทำงานชัดเจน ถ้า backend พัง หน้าเว็บยังโชว์อยู่ และเราสามารถเลือกใช้เครื่องมือที่เหมาะกับงานแต่ละส่วนได้

ข้อเสีย: ต้องจัดการเรื่องการเชื่อมต่อ และต้องคอยระวังเรื่อง version ของ api ให้ตรงกับหน้าเว็บ
...
```plaintext
# ตอบที่นี่
API Testing เช็คว่า โค้ดทำงานถูกตาม ไหม เช็คระหว่างทางใน github actions

Smoke Testing เช็คว่า เว็บตัวจริงเปิดติดและเชื่อมต่อได้ไหม เช็คหลังจากเอาขึ้น server ไปแล้ว

ตัวอย่างที่ CI ผ่านแต่ Smoke พัง โค้ดผ่านหมด แต่ลืมตั้งค่า config บน server จริง ทำให้ระบบคุยกับฐานข้อมูลไม่ได้
...
Helmet.js ปิดช่องโหว่พื้นฐานของเว็บ ป้องกันการแอบดูข้อมูลหัวข้อ http

express-rate-limit ป้องกันคนกดรัวๆ หรือยิงถล่มเว็บ

CORS อนุญาตเฉพาะหน้าเว็บที่เรากำหนดให้คุยกับ api ได้เท่านั้น

bcryptjs แปลงรหัสผ่านเป็นรหัสลับ ป้องกันการเห็นรหัสผ่านตัวจริงในฐานข้อมูล

npm audit เช็คว่าตัวเสริม ที่เราใช้มีช่องโหว่ความปลอดภัยไหม

TruffleHog แอบตรวจว่าเราเผลอใส่รหัสลับหรือ token ทิ้งไว้ในโค้ดหรือเปล่า

OWASP Dep-Check ตรวจความปลอดภัยของส่วนประกอบต่างๆ อย่างละเอียด
...
```plaintext
# ตอบที่นี่
เหตุผล เพื่อซ่อนรหัสลับ รหัสฐานข้อมูล ไม่ให้คนอื่นเห็นในโค้ด

ถ้าใส่ตรงๆ ถ้าเราเปิด repo เป็น public คนทั้งโลกจะเห็นรหัสเราทันที และถ้าจะเปลี่ยนรหัสต้องมานั่งแก้โค้ดและ push ใหม่ทุกครั้ง
...
```plaintext
# ตอบที่นี่
อธิบาย: develop -> staging -> main

ทำไมต้องแยก เพื่อให้เรามีที่ลองเล่น และที่ตรวจงานครั้งสุดท้าย ก่อนส่งให้ลูกค้าใช้จริง จะได้ลดโอกาสที่ลูกค้าจะเจอเว็บบั๊กได้ครับ
